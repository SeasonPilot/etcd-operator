...
var (
	etcdClusterCommonLabel = map[string]string{"app": "etcd"}
	etcdClusterLabelKey    = "etcd.season.io/cluster"
	etcdContainerName      = "etcd"
	etcdDataDirName        = "datadir"
)

func mutateSts(cluster *etcdv1alpha1.EtcdCluster, sts *appsv1.StatefulSet) {
	sts.ObjectMeta.Labels = etcdClusterCommonLabel
...
func newContainer(cluster *etcdv1alpha1.EtcdCluster) corev1.Container {
		...
		Env: []corev1.EnvVar{
			{
				...
				Value: strconv.Itoa(int(*cluster.Spec.Size)), // fixme: 是从 CR 中读取
			}, {
				Name: "MY_NAMESPACE",
				...
				Name:  "SET_NAME",
				Value: cluster.Name, // fixme: 是从 CR 中读取
			},
		},
		Command: []string{
			"/bin/sh",
...
func mutateSvc(cluster *etcdv1alpha1.EtcdCluster, svc *corev1.Service) {
	svc.ObjectMeta.Labels = etcdClusterCommonLabel
